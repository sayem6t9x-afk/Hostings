...
# API KEY FOR YSHSHOPMAILS
YSH_API_KEY = "6804564184237369dmp0dUFS0G4xAHQy"
...
def auto_cleanup_task():
    while True:
        try:
            ...
            logging.info("Auto-cleanup executed.")
...
def show_main_instruction(chat_id, message_id=None):
    ...
    instruction_text = (
        ...
        "You can also **manually send your credentials** at any time:\n\n"
        "🏢 **For Zoho / Alias:** `email@zohomail.com|AppPassword`\n"
        "🔴 **For Gmail (API):** `email@gmail.com|OrderID`\n"
...
def handle_query(call):
    ...
    if call.data == "action_menu":
        ...
        help_text = (
            "📖 **Bot Usage Guide:**\n\n"
            "1. Click **Buy Gmail** to automatically purchase and read OTPs.\n"
            "2. Or manually send your credentials in the chat.\n"
            "3. The bot strictly shows only FB OTP.\n"
    ...
    # --- BUY GMAIL LOGIC ---
        ...
        try:
            ...
            # Hit Create-Order API for Gmail (Service: Facebook)
            order_url = f"https://yshshopmails.com/v1/api/create-order.php?key={YSH_API_KEY}&service=facebook"
            ...
            if "mail" in resp and "order_id" in resp:
                ...
                # Auto Save to Database (Provider: gmail, Password: order_id)
                with sqlite3.connect('mail_bot.db', check_same_thread=False) as conn:
                    cursor = conn.cursor()
                ...
                # Automatically check inbox after buying
                time.sleep(2)
                fetch_and_send_emails(chat_id)
            else:
                bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=f"❌ **Failed to buy Gmail!**\nAPI Response: `{resp}`", parse_mode="Markdown")
                
        except Exception as e:
            bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=f"❌ **API Connection Error:** {e}", parse_mode="Markdown")

    # --- CHECK STOCK LOGIC (PENDING API) ---
    elif call.data == "action_check_stock":
        bot.answer_callback_query(call.id, "⚠️ Please provide the Stock API link in chat first!", show_alert=True)
...
# --- GLOBAL LISTENER: ALWAYS WAITING FOR MANUAL CREDENTIALS ---
...
def fetch_and_send_emails(chat_id, edit_message_id=None):
    ...
    try:
        if provider == 'gmail':
            # 🔴 DIRECT API BYPASS FOR GMAIL 🔴
            order_id = password
            api_url = f"https://yshshopmails.com/v1/api/check-otp.php?key={YSH_API_KEY}&id={order_id}"
            ...
            try:
                ...
                if "otp" in data and data["otp"]:
                    ...
                    raw_html = f"Facebook OTP Code: {otp_code}\n[Fetched via direct API bypassing Google Security]"
